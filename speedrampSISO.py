...
OUT_PATH = "/Users/anas/Desktop/Editor-test/out_ramp_slowfast.mp4"
...
def speed_at(u: float) -> float:
    """
    u in [0..1]
    Slow (1x) -> Fast (5x) -> Slow (1x) using sine squared:
    - u=0   => sin(0)=0    => MIN_SPEED
    - u=0.5 => sin(pi/2)=1 => MAX_SPEED
    - u=1   => sin(pi)=0   => MIN_SPEED
    """
    # CHANGED: swapped cos for sin to start slow
    return MIN_SPEED + (MAX_SPEED - MIN_SPEED) * (math.sin(math.pi * u) ** 2)
...
def run_siso(
    ...
    # --- SAFETY LOGIC ---
    # Even though start and end are slow, middle is FAST (5x)
